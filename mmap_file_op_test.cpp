...
// 内存映射的参数
const static largefile::MMapOption mmap_option = {10240000, 4096, 4096}; // 10兆,一页4KB
...
int main(void)
{
    ...
    {
        fprintf(stderr, "mmap_file %s failed. reason:%s\n", filename, strerror(errno));
        exit(-1);
    }
    ...
    if (ret == largefile::TFS_ERROR)
    {
        fprintf(stderr, "flush_file failed.reaseon:%s\n", strerror(errno));
    }
